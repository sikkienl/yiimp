...
if ($coin->rpcencoding == 'DCR' && isset($block['stx'])) {
	...
	foreach($block['stx'] as $txhash)
	{
		...
		echo "<td>";
		if(isset($stx['vout'][0]['scriptPubKey']) && Yii::$app->ConversionUtils->arraySafeVal($stx['vout'][0]['scriptPubKey'],'type') == 'stakesubmission')
			echo "Ticket";
		else foreach($stx['vin'] as $vin) {
			if (Yii::$app->ConversionUtils->arraySafeVal($vin,'blockheight') > 0) {
